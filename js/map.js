(function(){
	...
	.defer(d3.csv, "data/03-27-2020.csv")
	.await(ready);
	      ...
	      .attr("fill-opacity", .5)
			...
			svg
			  ...
			  .append("text")
			    ...
			    .text( function(d){ return d == 50000 ? d + ' case' : d } )
			    .style("font-size", 12)
			...
			function displayDetail(d) {
				...
				.html(function() {
					...
					return `<h4>${location}</h4>
						<p><span class="stats">Case confirmed</span> ${d.Confirmed}</p>
						<p><span class="stats">Deaths</span> ${d.Deaths}</p>
						<p><span class="stats">Recovered</span> ${d.Recovered}</p>
					`;})
					.style('opacity', 1);
				}
